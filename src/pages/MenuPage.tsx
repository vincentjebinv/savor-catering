...
import { Plus, Search, X, Image as ImageIcon,Settings } from 'lucide-react';
...
export default function MenuPage() {
  ...
  const activeDishes = dishes.filter(d => {
    // 🛡️ LOOSE EQUALITY TO HANDLE ANY STRING/NUMBER MISMATCH
    const matchesCategory = (d.category_id == activeCatId) || (d.categoryId == activeCatId);
  ...
  const handleAddItem = (dish: Dish, variety?: any) => {
    addItem({
      dishId: dish.id,
      categoryId: dish.category_id || (dish as any).categoryId,
      name_en: variety 
        ? (variety.name_en || variety.nameEn) 
        : (dish.name_en || dish.nameEn),
      name_ta: variety 
        ? (variety.name_ta || variety.nameTa) 
        : (dish.name_ta || dish.nameTa),
      varietyName: variety 
        ? (variety.name_en || variety.nameEn) 
        : 'Main Dish'
    });
  ...
  const getName = (item: any) => {
    if (!item) return '---';
    // Priority 1: Current language snake_case
    if (language === 'ta' && item.name_ta) return item.name_ta;
    if (language !== 'ta' && item.name_en) return item.name_en;
    // Priority 2: Fallback snake_case
    if (item.name_en) return item.name_en;
    if (item.name_ta) return item.name_ta;
    // Priority 3: CamelCase (Backend raw)
    if (item.nameEn) return item.nameEn;
    if (item.nameTa) return item.nameTa;
    return 'Unnamed';
  };
    ...
    if (dish.image_path) {
      ...
      return (
        <img 
          ...
          alt={dish.name_en || dish.nameEn} 
        />
      );
    }

    const libImage = getLibraryImage(dish.name_en || dish.nameEn, dish.name_ta || dish.nameTa);
    if (libImage) {
      return (
        <img 
          src={libImage} 
      ...
      <div className="sticky top-0 z-30 bg-white shadow-sm">
        ...
        <div className="flex h-16 items-center justify-between px-4 border-b border-gray-50">
          ...
          <div className="flex items-center justify-center gap-2 w-2/4">
            ...
            <span className="text-lg font-black text-slate-900 tracking-tight whitespace-nowrap">Solai Catering</span>
          </div>
                ...
                <div key={dish.id} className="group overflow-hidden rounded-2xl bg-white shadow-lg border border-gray-100 transition-all active:scale-95 h-fit">
                   ...
                   <div className="p-4">
                    ...
                    <p className="text-[10px] text-gray-400 truncate uppercase mt-0.5 font-bold">
                      {language === 'ta' ? (dish.name_en || dish.nameEn) : (dish.name_ta || dish.nameTa)}
                    </p>
        ...
        <div className="flex flex-col items-center justify-center py-32 px-10 text-center bg-slate-50 min-h-[calc(100vh-160px)]">
          ...
          <div className="flex flex-col gap-3 w-full max-w-xs">
            <button 
              onClick={async () => {
                setLoading(true);
                const { ...defaults } = await import('../utils/DefaultMenu');
                const { seedMenuData } = await import('../services/MenuService');
                const seedData = [
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm animate-in fade-in-25">
          <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[80vh]">
            ...
            <div className="overflow-y-auto p-3 space-y-3 bg-gray-50">
              {JSON.parse(viewingDish.varieties || '[]').map((v: any, idx: number) => {
                 return (
                  <div key={idx} className="flex items-center p-3 rounded-xl bg-white border border-gray-100 shadow-sm">
                    <div className="h-14 w-14 rounded-lg bg-gray-100 overflow-hidden flex-shrink-0 mr-4 border border-gray-200">
                      {v.image_path || getLibraryImage(v.name_en || v.nameEn, v.name_ta || v.nameTa) ? (
                        <img 
                          src={v.image_path ? (v.image_path.startsWith('http') ? v.image_path : Capacitor.convertFileSrc(v.image_path)) : getLibraryImage(v.name_en || v.nameEn, v.name_ta || v.nameTa)!} 
                          className="h-full w-full object-cover" 
                          alt={getName(v)}
                        />
                      ) : (
