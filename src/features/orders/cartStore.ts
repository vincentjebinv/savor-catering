...
export const useCartStore = create<CartState>((set, get) => ({
  ...
  restoreOrder: (historicOrder) => set((state) => {
    ...
    const newGroup: SelectionGroup = {
      id: newId,
      name: historicOrder.event_name || historicOrder.eventName || `Restored Order`,
      customerName: historicOrder.customer_name || historicOrder.customerName || '',
      eventDate: historicOrder.event_date || historicOrder.eventDate || '',
      eventTime: historicOrder.event_time || historicOrder.eventTime || '',
      address: historicOrder.address || '',
      notes: historicOrder.notes || '',
      items: (historicOrder.items || []).map((item: any) => ({
        dishId: item.dish_id || item.dishId || item.id || Date.now() + Math.random(),
        categoryId: item.category_id || item.categoryId || 0,
        name_en: item.dish_name_en || item.dishNameEn || '',
        name_ta: item.dish_name_ta || item.dishNameTa || '',
        quantity: item.quantity || 0,
        varietyName: item.variety_name || item.varietyName || 'Main Dish'
      })),
    };
