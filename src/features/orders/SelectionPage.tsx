              ...
              activeGroup.items.map(item => (
                <div key={`${item.dishId}-${item.varietyName}`} className="flex items-center rounded-xl bg-white p-4 shadow-sm border border-gray-100">
                  ...
                  <div className="flex items-center gap-2">
                    <input type="number" placeholder="0" value={item.quantity === 0 ? '' : item.quantity} onChange={(e) => { const newQty = parseInt(e.target.value) || 0; const delta = newQty - item.quantity; updateQuantity(item.dishId, item.varietyName, delta); }} className="w-16 h-10 border rounded-lg text-center font-bold text-slate-800 focus:border-green-500 outline-none" />
